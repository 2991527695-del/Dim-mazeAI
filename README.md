...
Algorithm_Library_Create.push_back({"生成算法名字","介绍",std::bind(&Maze_AI::你的算法函数名, this)});
Algorithm_Library_Search.push_back({"搜索算法名字","介绍",std::bind(&Maze_AI::你的算法函数名, this)});
}
```
**不要忘记头文件添加()*
...
# 自带算法
- DFS，BFS(创造)，双向DFS，A
- 都不是抄作业来的 这些()甚至有一些实现之前都不知道他叫这个名字
- 所以看起来可能有点抽象 但是他是实打实打算可以跑QWQ 你也可以删除使用你自己的算法